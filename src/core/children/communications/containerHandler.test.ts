...
import { MockContainerCommunicationHandler } from "../../../../test/mocks/mockContainerCommunicationHandler";
import { values_falsies } from "../../../../test/utils";
import { createCustomEvent } from "../../../dom/document/createCustomEvent";
import { CommunicationsEvent, CommunicationsEventKind, ContainerCommunicationHandlerMethods } from "../../../index";
...
describe("ContainerCommunicationHandler", () => {
  ...
  it("should not fail if calling send after dispose", () => {
    _handler.dispose();
    expect(() => _handler.send(new CommunicationsEvent(CommunicationsEventKind.BeforeDispose))).not.to.throw();
  });
  ...
  values_falsies.forEach((f) => {
    it(`should not fail if calling handleEventCore without any methods("${f}")`, () => {
      ...
      expect(() =>
        hander.handleEventCore(new CommunicationsEvent(CommunicationsEventKind.BeforeDispose)),
      ).not.to.throw();
    });
  });
