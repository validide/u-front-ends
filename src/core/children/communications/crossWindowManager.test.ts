...
import { MockCrossWindowCommunicationsManager } from "../../../../test/mocks/mockCrossWindowCommunicationsManager";
import { getDelayPromise, values_falsies } from "../../../../test/utils";
import { createCustomEvent } from "../../../dom/document/createCustomEvent";
import { CommunicationsEvent, CommunicationsEventKind, CrossWindowCommunicationDataContract } from "../../../index";
...
describe("CrossWindowCommunicationsManager", () => {
  ...
  it("calling send to an endpoint will send", async () => {
    ...
    const data = new CrossWindowCommunicationDataContract<CommunicationsEvent>(_eventType, det);
    _win.addEventListener("message", (e: Event) => {
      events.push(e);
    });
  ...
  it("read an event and return null if the origin is not the same", () => {
    ...
    const data = new CrossWindowCommunicationDataContract<CommunicationsEvent>(_eventType, evt);
