...
import { CommunicationsEvent, CommunicationsEventKind } from "../../../index";
...
describe("CommunicationsManager", () => {
  ...
  it("calling dispose multiple times does not fail", () => {
    expect(() => {
      _mngr.dispose();
      _mngr.dispose();
      (_mngr as unknown as { disposed?: boolean }).disposed = false;
      _mngr.dispose();
    }).not.to.throw();
  });
  ...
  it("calling initialize without an inbound enpoint does not fail but does not receive", () => {
    const mngr = new MockCommunicationsManager(
  ...
  it("will handle event if handler is set", async () => {
    ...
    _mngr.initialize();
    _mngr.setEventReceivedCallback((e: CommunicationsEvent) => {
      evts.push(e);
    });
