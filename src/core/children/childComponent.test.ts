...
import {
  MockChildComponent,
  type MockContainerCommunicationHandler,
} from "../../../test/mocks/mockChildComponentFactory";
import { ChildComponentOptions, type ContainerCommunicationHandler, noop, RootComponentFacade } from "../../index";
...
describe("ChildComponent", () => {
  ...
  it('"contentBeginDisposed" calls parent only once', async () => {
    ...
    const mock = new MockChildComponent(
      _win,
      _options,
      new RootComponentFacade(() => {
        disposedRootCalls++;
      }),
    );
    await mock.initialize();
  ...
  it("multiple dispose calls result in single requestContentDispose", async () => {
    ...
    try {
      const all = Promise.all([mock.disposeCore(), mock.disposeCore()]);
  ...
  it("receives events from content", async () => {
    ...
    _options.tag = "ce-dipose-tests";
    _options.handlers.mounted = () => {
      mounted = true;
    };
    _options.handlers.beforeUpdate = () => {
      beforeUpdate = true;
    };
    _options.handlers.updated = () => {
      updated = true;
    };
    _options.handlers.data = (evt) => {
      data = evt.data;
    };
