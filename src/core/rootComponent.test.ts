...
import { type MockChildComponent, MockChildComponentFactory } from "../../test/mocks/mockChildComponentFactory";
import { getDelayPromise, values_falsies } from "../../test/utils";
import { ChildComponentOptions, RootComponent, RootComponentOptions } from "..";
...
describe("RootComponent", () => {
  ...
  describe("remove child", () => {
    it("should not throw if ID does not exist", () => {
      const root = new RootComponent(_win, _options);
      expect(async () => await root.removeChild("some_id_that_does_not_exit")).not.to.throw();
    });
