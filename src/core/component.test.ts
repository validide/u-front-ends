...
class StubComponent extends Component {
  ...
  protected async initializeCore(): Promise<void> {
    ...
    if (this.throwError) throw this.testError;
...
describe("Component", () => {
  ...
  describe("Protected", () => {
    ...
    it(`callHandler throws when using the ${ComponentEventType.Error}`, () => {
      ...
      expect(() => comp.callHandler(ComponentEventType.Error)).to.throw(
        `For calling the "${ComponentEventType.Error}" handler use the "callErrorHandler" method.`,
      );
    });
  ...
  describe("Initialize", () => {
    ...
    values_falsies.concat(["some-random-id"]).forEach((f) => {
    ...
    it("Should fire the following events", async () => {
      ...
      const handler = (evt: ComponentEvent) => {
        ...
        if (!events[evt.type.toString()]) {
          events[evt.type.toString()] = [] as ComponentEvent[];
        }
