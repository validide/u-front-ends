((window, ufe, undefined) => {
  ...
  mainNavBarScript.skip = () => !!app.inWindow["MainNavBarComponent"];
  ...
  mainNavBarCustomElScript.skip = () => !!window.customElements.get("main-nav-bar");
  ...
  function init() {
    ...
    const rootMountProm = mfe.initialize().then((root) => root.mount());

    Promise.all([
      rootMountProm.then(async (root) => {
